...
def ridership():
    try:
        model_path = "backend/utils/models.pickle"
        ...
        for station in manhattan_unique:
            try:
                ...
                if station in models:
                    ...
                    prediction = round(abs(predict(coef, station_df["ridership"].values)), 2)
                    ridership_pred[station] = prediction
                else:
                    # No model for this station, use average of available data
                    ridership_pred[station] = round(abs(station_df["ridership"].mean()), 2)
            except Exception as e:
